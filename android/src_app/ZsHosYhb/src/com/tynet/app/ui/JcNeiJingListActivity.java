...
import com.tynet.app.adapter.ListViewJcNeiJingAdapter;
...
public class JcNeiJingListActivity extends ListActivity{

	private GenericTask mInitTask;
	private BookPlatApplication appContext;
	private JcNeiJingList m_obj;
	private List<JcNeiJing> m_itemList = new ArrayList<JcNeiJing>();
	private ListViewJcNeiJingAdapter m_adapter;
	private String bah;	
	@Override
...
//		bah = "07161171";
		...
		m_adapter = new ListViewJcNeiJingAdapter(this, m_itemList,
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		Intent i = new Intent(this,JcNeiJingInfoActivity.class);
		JcNeiJing item = (JcNeiJing)m_itemList.get(position);
	...
	private class InitTask extends GenericTask {
		...
		protected TaskResult _doInBackground(TaskParams... params) {
			...
			try {
				...
				m_obj = appContext.getJcNeiJingList(paramJSON);
				if (m_obj.isSuccess()) {
					m_itemList.clear();
	...
	private TaskListener mInitTaskListener = new TaskAdapter() {
		...
		private void onInitBegin() {
			TaskFeedback.getInstance(TaskFeedback.DIALOG_MODE,
					JcNeiJingListActivity.this).start(
					getString(R.string.activity_init));
		}

		private void onInitSuccess() {
			TaskFeedback.getInstance(TaskFeedback.DIALOG_MODE,
					JcNeiJingListActivity.this).success();
			m_adapter.notifyDataSetChanged();
		}

		private void onInitFailure(String reason) {
			TaskFeedback.getInstance(TaskFeedback.DIALOG_MODE,
					JcNeiJingListActivity.this).failed(reason);
		}
