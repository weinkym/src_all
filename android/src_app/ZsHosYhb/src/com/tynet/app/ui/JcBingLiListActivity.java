...
import com.tynet.app.adapter.ListViewJcBingLiAdapter;
import com.tynet.app.bean.JcBingLi;
...
public class JcBingLiListActivity extends ListActivity{

	private GenericTask mInitTask;
	private BookPlatApplication appContext;
	private JcBingLiList m_obj;
	private List<JcBingLi> m_itemList = new ArrayList<JcBingLi>();
	private ListViewJcBingLiAdapter m_adapter;
	private String bah;	
	@Override
...
//		bah = "07161171";
		...
		m_adapter = new ListViewJcBingLiAdapter(this, m_itemList,
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		Intent i = new Intent(this,JcBingLiInfoActivity.class);
		JcBingLi item = (JcBingLi)m_itemList.get(position);
		i.putExtra("lsh",item.getBlh());
		startActivity(i);
	}
	...
	private class InitTask extends GenericTask {
		...
		protected TaskResult _doInBackground(TaskParams... params) {
			...
			try {
				...
				m_obj = appContext.getJcBingLiList(paramJSON);
				if (m_obj.isSuccess()) {
					m_itemList.clear();
	...
	private TaskListener mInitTaskListener = new TaskAdapter() {
		...
		private void onInitBegin() {
			TaskFeedback.getInstance(TaskFeedback.DIALOG_MODE,
					JcBingLiListActivity.this).start(
					getString(R.string.activity_init));
		}

		private void onInitSuccess() {
			TaskFeedback.getInstance(TaskFeedback.DIALOG_MODE,
					JcBingLiListActivity.this).success();
			m_adapter.notifyDataSetChanged();
		}

		private void onInitFailure(String reason) {
			TaskFeedback.getInstance(TaskFeedback.DIALOG_MODE,
					JcBingLiListActivity.this).failed(reason);
		}
