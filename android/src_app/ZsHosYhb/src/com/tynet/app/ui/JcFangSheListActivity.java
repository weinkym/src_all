...
import com.tynet.app.adapter.ListViewJcFangSheAdapter;
import com.tynet.app.bean.JcFangShe;
...
public class JcFangSheListActivity extends ListActivity{

	private GenericTask mInitTask;
	private BookPlatApplication appContext;
	private JcFangSheList m_obj;
	private List<JcFangShe> m_itemList = new ArrayList<JcFangShe>();
	private ListViewJcFangSheAdapter m_adapter;
	private String bah;	
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		...
		m_adapter = new ListViewJcFangSheAdapter(this, m_itemList,
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		Intent i = new Intent(this,JcFangSheInfoActivity.class);
		JcFangShe item = (JcFangShe)m_itemList.get(position);
//		i.putExtra("lsh",item.getLsh());
		i.putExtra("lsh",item.getCheckNumber());		
		System.out.println("lsh" + item.getCheckNumber());
		startActivity(i);
	}
	...
	private class InitTask extends GenericTask {
		...
		protected TaskResult _doInBackground(TaskParams... params) {
			...
			try {
				...
				m_obj = appContext.getJcFangSheList(paramJSON);
				if (m_obj.isSuccess()) {
					m_itemList.clear();
	...
	private TaskListener mInitTaskListener = new TaskAdapter() {
		...
		private void onInitBegin() {
			TaskFeedback.getInstance(TaskFeedback.DIALOG_MODE,
					JcFangSheListActivity.this).start(
					getString(R.string.activity_init));
		}

		private void onInitSuccess() {
			TaskFeedback.getInstance(TaskFeedback.DIALOG_MODE,
					JcFangSheListActivity.this).success();
			m_adapter.notifyDataSetChanged();
		}

		private void onInitFailure(String reason) {
			TaskFeedback.getInstance(TaskFeedback.DIALOG_MODE,
					JcFangSheListActivity.this).failed(reason);
		}
